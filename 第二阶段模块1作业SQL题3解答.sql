CREATE DATABASE IF NOT EXISTS lianxi03 DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;
USE lianxi03;
#学生表
...
SELECT s.id, s.name, count(c.name), sum(sc.score)
FROM lianxi03.student s
         JOIN student_course sc ON s.id = sc.student_id
...
FROM lianxi03.student s
         JOIN student_course sc ON s.id = sc.student_id
WHERE sc.course_id = (SELECT c.id
                      FROM teacher t
...
SELECT a.id, a.name
FROM (SELECT s.id, s.name, count(s.name)
      FROM lianxi03.student s
               JOIN student_course sc ON s.id = sc.student_id
      GROUP BY s.id, s.name
      HAVING count(s.name) < 3) a;

DROP DATABASE IF EXISTS lianxi03;